...
class value:
    ...
    def get_rate(self, num: str):
        ...
        try:
            ...
            total = get_content_by_xpath(self.lazy, f'/html/body/div[3]/div[1]/table/tbody/tr[{int(num)+1}]/td[5]/text()')[0]
            rate = get_content_by_xpath(self.lazy, f'/html/body/div[3]/div[1]/table/tbody/tr[{int(num)+1}]/td[9]/text()')[0]
            if time == '' or total == '' or rate == '':
                return None
            return (time, total, rate)
        except:
            return None
    ...
    def query(self, s: str) -> list[tuple[str, str]]:
        try:
            ...
            for page in pages:
                ...
                for fund in funds:
                    try:
                        info = str(fund.xpath('div/a/text()')[0]).encode('latin1').decode('gb2312')
                    except:
                        continue
                    code = info[1:7]
                    name = info[8:]
